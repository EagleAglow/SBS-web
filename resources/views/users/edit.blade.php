			...
			<div class="card mt-5 shadow">
				<div class="card-header"><h5>Edit User: {{$user->name}}</h5>
                    ...
                    <form method="POST" action="{{ route('users.update', $user->id) }}" accept-charset="UTF-8">
                        ...
                        <div class="form-group row">
                            <label for="bidder_primary_order" class="col-md-4 col-form-label text-md-right">{{ __('Seniority Order') }}</label>
                            <div class="col-md-6">
                                <input id="bidder_primary_order" type="text" class="form-control @error('bidder_primary_order') is-invalid @enderror" name="bidder_primary_order" value="{{ old('bidder_primary_order') ? old('bidder_primary_order') : $user->bidder_primary_order }}" autocomplete="bidder_primary_order" autofocus>
                                @error('bidder_primary_order')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                                @enderror
