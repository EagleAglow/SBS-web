...
// Potential problem: using arrays seems to run into a PHP memory limit, which probably can be configured
// The default maximum array limit may be 32768 elemnts - for 67 items per record, that is just under 490 lines.
// so, export is batched by schedule, hopefully less than 490 rows!!

        // an array for a header
        $first = DB::select(DB::raw("SELECT 'SCHEDULE' as title, 'SCHEDULE_ID', 'GROUP' as code,
                                        'GROUP_ID' as line_group_id, 'LINE', 'BLACKOUT' as blackout,
                                        'NEXUS' as nexus, 'BARGE' as barge, 'OFFSITE' as offsite, 'COMMENT' as comment,
        ...
        $blocks = Schedule::select('id')->orderBy('start')->get();
        foreach ($blocks as $block){
            $lines = DB::table('schedule_lines')->where('schedule_id','=', $block->id)
                    ...
                    ->select('title', 'schedule_id', 'code', 'line_group_id as group_id', 'line',
