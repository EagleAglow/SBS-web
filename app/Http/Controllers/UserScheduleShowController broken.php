...
// skip lines that have been taken (have an assigned 'user_id') or are 'blackout' - works, still need to sort tag order
//$schedule_lines = ScheduleLine::where('schedule_id',$id)->where('blackout',0)->whereNull('user_id')->whereIn('line_group_id',$list)->paginate(5); //Get first 5 ScheduleLines

// close to the right thing??
//$schedule_lines = ScheduleLine::where('schedule_id',$id)->where('blackout',0)->whereNull('schedule_lines.user_id')->whereIn('line_group_id',$list)
//->join('picks','schedule_lines.id','=','picks.schedule_line_id')->select('schedule_lines.*', 'picks.rank')->orderByDesc('picks.rank')
//->paginate(5); //Get first 5 ScheduleLines


// try for lines not yet taken, in order of pick rank
//$schedule_lines = DB::table('schedule_lines')->where('schedule_id',$id)->whereNull('schedule_lines.user_id')->whereIn('line_group_id',$list)->leftJoin('picks','schedule_lines.id','=','picks.schedule_line_id')->orderBy('picks.rank')->paginate(5); //Get first 5 ScheduleLines

//$schedule_lines = DB::table('schedule_lines')->where('schedule_id',$id)->whereIn('line_group_id',$list)->rightJoin('picks','schedule_lines.id','=','picks.schedule_line_id') ->select('schedule_lines.*', 'picks.rank')->orderBy('picks.rank')->paginate(5); //Get first 5 ScheduleLines

//$schedule_lines = DB::table('schedule_lines')->where('schedule_id',$id)->whereIn('line_group_id',$list)
//->leftJoin('picks','schedule_lines.id','=','picks.schedule_line_id')->orderBy('picks.rank') 
//->select('schedule_lines.*', 'picks.rank')->paginate(5); //Get first 5 ScheduleLines
...
//->join('picks','schedule_lines.id','=','picks.schedule_line_id')->where('picks.user_id','=',4)
->join('picks','schedule_lines.id','=','picks.schedule_line_id')->select('schedule_lines.*', 'picks.rank')->orderByDesc('picks.rank')
